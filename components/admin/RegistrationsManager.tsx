...
interface RegistrationsManagerProps {
  /** When set, scope all data and filters to this tournament only. */
  fixedTournamentId?: string;
  /** Render inside tournament console (no AdminLayout wrapper). */
  embedded?: boolean;
}
...
export function RegistrationsManager({
  ...
  const [selectedTournament, setSelectedTournament] = useState<string>(fixedTournamentId ?? 'all');
  ...
  const loadTournaments = async () => {
    try {
      ...
      if (fixedTournamentId) {
        tournamentsData = tournamentsData.filter((t) => t.id === fixedTournamentId);
      }
  ...
  const loadRegistrations = async () => {
    try {
      ...
      const tournamentsToLoad = fixedTournamentId
        ? tournaments.filter((t) => t.id === fixedTournamentId)
        : tournaments;
  ...
  const content = (
    <>
      <div
        className={
          embedded
            ? 'flex h-[calc(100dvh-7rem)] min-h-[420px] min-w-0 max-w-full flex-col overflow-hidden'
            : 'flex h-[calc(100dvh-3.5rem)] min-h-0 min-w-0 max-w-full flex-col overflow-hidden p-4'
        }
      >
        ...
        <Card className="flex min-h-0 flex-1 flex-col gap-2 overflow-hidden py-3">
          <CardHeader className="shrink-0 pb-0">
            <div className="flex flex-nowrap items-center gap-2 overflow-x-auto pb-1">
              ...
              <Button
                onClick={() => {
                  if (fixedTournamentId) setSelectedImportTournament(fixedTournamentId);
  ...
  return <AdminLayout moduleName="Registrations">{content}</AdminLayout>;
}
